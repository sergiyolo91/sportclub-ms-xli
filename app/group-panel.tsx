...
import styles from './group-panel.module.css'
...
export default function GroupPanel({ userId, group, onLogout }: { userId: string; group: Group; onLogout: () => void }) {
  ...
  return <section className="screen-pad">
    ...
    <div className="group-code">
      ...
      <button className={styles.copyCode} onClick={copyCode}><Copy size={15}/>{copied ? 'Kopiert' : 'Code kopieren'}</button>
    </div>

    {group.role === 'ADMIN' && <>
      <button className={styles.adminAction} onClick={() => setShowChallenge(v => !v)}><Flame size={18}/>{showChallenge ? 'Challenge schließen' : 'Neue Challenge'}</button>
      {showChallenge && <div className={styles.adminForm}>
        ...
        <div className={styles.adminGrid}><div><label>Zielwert</label><input inputMode="numeric" value={target} onChange={e => setTarget(e.target.value)} /></div><div><label>Laufzeit (Tage)</label><input inputMode="numeric" value={days} onChange={e => setDays(e.target.value)} /></div></div>
    ...
    <div className={`list-card ${styles.memberCard}`}>
      ...
      {members.map(member => <div className="row" key={member.user_id}>
        ...
        <div className={`mini-avatar ${member.user_id === userId ? styles.me : ''}`}>{member.display_name.slice(0,1).toUpperCase()}</div>
      </div>)}
